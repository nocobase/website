...
|     **比較項目**     | **Vibe Coding**                                                                                        | **NocoBase + AI**                                                                       |
| :------------------: | :----------------------------------------------------------------------------------------------------- | :-------------------------------------------------------------------------------------- |
|   **初回生成結果**   | 今回のテストではフロントエンドのプロトタイプに近く、追加のデータ構造や業務ロジックの実装が必要でした。 | 今回のテストでは、業務データの登録、関連付け、操作まで実行できる状態でした。            |
|   **データモデル**   | データベース構造、API、データ関係をプロジェクト内で実装する必要があります。                            | 商品、購買、在庫などの関係は、統一されたデータモデル上に直接構築されます。              |
| **権限とデータ範囲** | 業務ルールに基づいて権限ロジックを開発・テストする必要があります。                                     | ロール、操作権限、データ範囲を直接設定できます。                                        |
|   **業務プロセス**   | ステータス遷移、承認、業務自動化は追加開発が必要です。                                                 | ワークフローを利用して直接設定・調整できます。                                          |
|     **要件変更**     | データベース、API、画面、入力チェック、テストなど複数の箇所に影響する可能性があります。                | 主にデータモデル、画面、権限、ワークフローを中心に調整します。                          |
|     **拡張方法**     | コードを直接変更・拡張します。                                                                         | 設定による拡張に加え、必要に応じて JS、プラグイン、コードによるカスタマイズも可能です。 |
|   **継続的な保守**   | 開発者と完全なコードベースへの依存度が高くなります。                                                   | 多くの業務変更は設定で対応でき、複雑な要件のみコード層で対応できます。                  |
