...
| 对比项         | Vibe Coding                                            | NocoBase + AI                                          |
| -------------- | ------------------------------------------------------ | ------------------------------------------------------ |
| 第一轮结果     | 本次测试中更接近前端原型，还需要继续补充数据和业务逻辑 | 本次测试中已经可以录入、关联和操作业务数据             |
| 数据模型       | 数据库结构、接口和数据关系需要在项目中实现             | 商品、采购、库存等关系直接建立在统一的数据模型中       |
| 权限与数据范围 | 根据业务规则编写并测试权限逻辑                         | 可配置角色、操作权限和数据范围                         |
| 业务流程       | 状态流转、审批和自动化需要继续开发                     | 可直接基于工作流配置和调整                             |
| 需求变化       | 可能同时影响数据库、接口、页面、校验和测试             | 主要围绕数据模型、页面、权限和工作流调整               |
| 扩展方式       | 直接修改和扩展代码                                     | 既可以通过配置扩展，也可以继续使用 JS、插件和代码定制  |
| 后续维护       | 更依赖开发人员和完整的代码工程                         | 很多业务调整可以继续通过配置完成，复杂需求再进入代码层 |
