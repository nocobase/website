...
NocoBase 是一个数据模型驱动的开源、AI 驱动的无代码平台，采用插件化架构，所有功能都可以通过插件扩展。它不仅支持细粒度的权限控制（RBAC）、字段级 / 行级数据隔离，以及工作流自动化，还能在 AI 驱动能力的辅助下，通过 API 和插件接入不同的数据源（数据库、API、第三方服务），让资产数据的集中管理更加灵活。由于完全开源，企业还可以根据需要自主拓展功能，在保持低成本的同时实现高度定制。
