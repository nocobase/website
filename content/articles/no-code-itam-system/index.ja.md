...
NocoBase は、データモデル駆動のオープンソース AI ノーコードプラットフォームで、プラグイン型アーキテクチャを採用しており、すべての機能をプラグインによって拡張できます。RBAC による細粒度の権限管理、フィールド単位／行単位のデータ分離、ワークフロー自動化に対応しており、AI の機能を活用して API やプラグイン経由でデータベース、API、外部サービスなど多様なデータソースと連携できます。完全にオープンソースであるため、低コストを維持しながら要件に応じた高度なカスタマイズが可能です。
