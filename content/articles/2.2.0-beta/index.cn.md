...
`/v/` 是全新的独立前端入口，并且会独立构建。这个分支会完全移除 V1 的历史遗留内容，只保留面向 V2 的能力。这个分支将会是 NocoBase 团队后续主要迭代的分支。

我们建议，生产环境如果全部是以 v2 Page 为主的用户，可以考虑使用 `/v/` 分支作为默认入口，后续的一些新功能迭代将能直接使用；
如果是以 v2 Page 为主，但是有少部分 v1 Page 的用户，可以考虑先将 v1 Page 迁移到 v2 Page 之后，再考虑使用 `/v/` 分支作为默认入口；
...
# v 分支路径
https://nocobase.example.com/v/

# 原本的路径
https://example.com/nocobase/

# v 分支路径
https://example.com/nocobase/v/
```
