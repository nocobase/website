---
title: "¡Actualización importante! NocoBase V1.0 lanzada oficialmente"
description: "Descubre las principales novedades de NocoBase V1.0, incluyendo nuevas funciones, plugins y mejoras significativas de rendimiento. Conoce cómo este lanzamiento histórico mejora la estabilidad, seguridad y funcionalidad para desarrolladores. Obtén información detallada sobre el nuevo editor de temas, las mejoras en la visualización de datos y el registro de cambios completo. Comprende el proceso de actualización, especialmente si usas plugins SSO, y explora el impacto de estos cambios en tus proyectos."
---

## Hito V1.0

Después de 3 años de desarrollo, NocoBase ha alcanzado su primera actualización de versión importante, pasando de la versión 0.x a la 1.0. Esto marca un hito significativo.

* Durante la fase 0.x: La API principal y las funciones experimentaron cambios rápidos, y cada nueva versión podía introducir cambios incompatibles.
* Durante la fase 1.x: La API principal se ha estabilizado, y NocoBase se centrará en añadir nuevos plugins, optimizar la seguridad y mejorar el rendimiento manteniendo la estabilidad.

## Nuevas Funciones

* El editor de temas puede configurar el relleno de páginas y modales, el espaciado entre bloques y el radio de las esquinas de los bloques.
* Al añadir bloques, las colecciones admiten filtrado.
* Los registros de relaciones admiten ordenación.
* Interactividad mejorada de la visualización de datos.
* Soporte para añadir bloques de gráficos en dispositivos móviles.
* Los bloques de filtro de gráficos admiten establecer el ámbito de datos del campo.
* Añadir más variables, [ver detalles](https://docs.nocobase.com/handbook/ui/variables).
* Se pueden añadir bloques de cualquier colección en ventanas emergentes.
* Flujos de trabajo: El "evento posterior a la acción" puede ser activado por botones dentro de bloques de asociación.
* Actualización automática de los datos en el contenedor padre cuando los datos cambian dentro de una ventana emergente.
* Mejoras significativas de rendimiento en los bloques de tabla.
* Aumento sustancial en la cobertura de pruebas.

## Nuevos Plugins

* Campo: Markdown (Vditor)
  * Se utiliza para almacenar Markdown y renderizarlo usando el editor Vditor, soporta la sintaxis común de Markdown y admite la subida de imágenes, grabaciones, etc. También permite el renderizado instantáneo, donde lo que ves es lo que obtienes.
* Comentarios
...
Los siguientes plugins ya no se proporcionan como versión de código abierto (los usuarios que los hayan estado utilizando en entornos de producción, por favor, contáctenos para las actualizaciones):

* @nocobase/plugin-auth-cas: Autentica la identidad mediante el protocolo CAS.
* @nocobase/plugin-auth-odic: Autentica la identidad mediante el protocolo OIDC (OpenID Connect).
* @nocobase/plugin-auth-saml: Autentica la identidad mediante SAML 2.0.

Los siguientes plugins han sido obsoletos y se eliminarán en versiones futuras:

* @nocobase/plugin-audit-logs: Obsoleto, temporalmente no eliminado, no se instalará en la nueva versión.
* @nocobase/plugin-snapshot-field: Obsoleto, temporalmente no eliminado, no se instalará en la nueva versión.
* @nocobase/plugin-charts: Use @nocobase/plugin-data-visualization en su lugar.
* @nocobase/plugin-excel-formula-field: Use @nocobase/plugin-field-formula en su lugar.
* @nocobase/plugin-math-formula-field: Use @nocobase/plugin-field-formula en su lugar.
* @nocobase/plugin-ui-routes-storage: Obsoleto, las rutas del frontend se pueden extender directamente en el frontend.

Para la lista completa de plugins, consulte: [https://www.nocobase.com/plugins.html](https://www.nocobase.com/plugins.html)

## Comentarios de Código

Para proporcionar una mejor experiencia de desarrollo, hemos añadido comentarios a algunas API especiales:

* `@internal`: Normalmente se utiliza para identificar detalles de implementación interna o métodos que no están diseñados para uso público, sino que existen para uso interno dentro del código.
* `@experimental`: Se utiliza para identificar características o API experimentales. Estas API aún están en fases de desarrollo y prueba, pueden sufrir cambios significativos, o incluso ser eliminadas o reemplazadas en versiones futuras.
* `@deprecated`: Se utiliza para identificar características, métodos o API obsoletos en el código. Aunque todavía están disponibles, pueden ser eliminados en versiones futuras, o puede haber mejores soluciones alternativas disponibles.

## Acuerdo de Licencia

NocoBase tiene una licencia dual bajo AGPL-3.0 y licencias comerciales. Para más detalles, consulte el [Acuerdo de Licencia de NocoBase](https://cn.nocobase.com/agreement-cn.html).

## Guía de Actualización

* ¡Siempre haga una copia de seguridad de su base de datos antes de actualizar! ¡Siempre haga una copia de seguridad de su base de datos! ¡Siempre haga una copia de seguridad de su base de datos!
* También puede hacer una copia de seguridad de todo el código del proyecto.

Dado que los plugins SSO Auth: CAS, Auth: OIDC, Auth: SAML se han eliminado del código de código abierto, la actualización se dividirá en dos casos.

### Si no está utilizando plugins SSO

Siga el [método convencional](https://docs.nocobase.com/welcome/getting-started/upgrading) para la actualización.

### Si está utilizando plugins SSO

Siga estos pasos para actualizar:

#### 1. Actualice la Aplicación

Consulte la [Descripción General de Actualización de NocoBase](https://docs.nocobase.com/welcome/getting-started/upgrading) y actualice NocoBase a la última versión.

Si anteriormente habilitó los plugins CAS, OIDC, SAML, verá los siguientes mensajes durante la actualización:

Símbolo del sistema de actualización por línea de comandos:

![20240428212151](https://static-docs.nocobase.com/20240428212151.png)

Símbolo del sistema de la versión Docker:

![20240428194926](https://static-docs.nocobase.com/20240428194926.png)

#### 2. ¿Eliminar Plugins u Obtener la Versión 1.0 del Plugin?

***Si elige eliminar los plugins y continuar con la actualización:***

Elimine los plugins según las indicaciones

```
# Aplicación principal
yarn pm remove cas oidc saml --force
# Si es una sub-aplicación, añada el parámetro --app
yarn pm remove cas oidc saml --force --app=sub-app1
```

Continúe con la actualización

```
yarn nocobase upgrade
```

***Si elige actualizar los plugins a la versión 1.0***

Póngase en contacto con el equipo de NocoBase para obtener la versión 1.0 de los plugins y continúe con la actualización.

#### 3. Proceso de Actualización de Plugins CAS, OIDC, SAML

En este punto, la interfaz de la aplicación ya no es accesible, por lo que debemos actualizar manualmente.

1. Inicie sesión con su cuenta en [service.nocobase.com](https://service.nocobase.com/) para descargar la última versión de los plugins.
2. Descomprima los plugins en el directorio especificado
   * Plugin CAS descomprimir en `./storage/plugins/@nocobase/plugin-auth-cas`
   * Plugin OIDC descomprimir en `./storage/plugins/@nocobase/plugin-auth-oidc`
   * Plugin SAML descomprimir en `./storage/plugins/@nocobase/plugin-auth-saml`
3. Actualice la aplicación
   * Para la versión Docker, simplemente reinicie el contenedor
   * Para la versión de código fuente o create-nocobase-app
     1. Descargue las dependencias `yarn install`
     2. Ejecute el comando de actualización `yarn nocobase upgrade`
