...
* **コストの懸念**：Salesforceは売買、投資営業部がメインで利用しており、他の部署ではCRMとしての利用していない状態でした。今回の目的であるバックオフィス系の業務をSalesforceで利用するとなると、CRMを利用しないユーザーにライセンスを付与しなくてはいけません。
* **カスタマイズの制限：** 機能は豊富ですが、Salesforceのインターフェースやプロセスのカスタマイズオプションは実質的に限られていました。これにより、HouseWellの多様な事業分野の特定のニーズに正確に応えることが難しい状況でした。
...
詳細な機能テストとコスト評価の結果、チームはNocoBaseが機能面で十分に柔軟であるだけでなく、そのオープン性と拡張性は大変優れており、企業成長と共に成長できるシステムであると確信しました。
...
### フェーズ2：統合と不動産管理パッケージの開発

NocoBase導入前、HouseWellはすでに顧客管理にSalesforceを利用しており、全ての顧客詳細と取引記録がそこに保存されていました。NocoBaseの利用が進むにつれ、同社はNocoBaseが柔軟な人事管理に適しているだけでなく、CRMシステムの構築にも十分対応可能であることに徐々に気づきました。そこで、技術チームは迅速にシステム統合に着手しました。
...
* NocoBaseのカスタマイズ可能なデータモデルとページレイアウトを利用し、弊社の顧客管理のノウハウを活かして、不動産会社向けに不動産管理パッケージをNocoBaseで開発している状況です。（近日提供予定）
