...
**NocoBase** は、「データモデリングによるシステム構築」を中核思想とする、オープンソースでプラグイン型の AI ノーコード／ローコードプラットフォームです。AI の機能を活用することで、設定や利用の効率を高めています。PostgreSQL はデフォルトで統合されている主要なデータベースの一つです。単に PostgreSQL に接続するだけのプラットフォームとは異なり、NocoBase は開発者にとって扱いやすい構造レベルの制御機能を提供し、データベース設計ツールのようにフロントエンド上で業務構造を直接定義できます。
