...
【问】我先启动了 NocoBase 系统，之后才在设置里填写的 License Key，为什么授权成功了，但商业插件还是没有出现？

【答】这是预期的行为，也是一个非常常见的问题。NocoBase 的设计是在**服务启动时**检查授权并加载相应的插件。如果您是在系统已经运行的状态下完成的授权，那么需要**重启一次 NocoBase 服务**来触发商业插件的下载和激活。

简单来说，授权就像是给了 NocoBase 一张“通行证”，但它需要重新“进门”（重启）时才会检查并使用这张通行证。

请根据您的部署方式，执行相应的重启操作：

* **对于 Docker 部署**：执行重启命令 `docker compose restart app`。
* **对于源码或 `create-nocobase-app` 部署**：先在终端里停止当前运行的服务 (通常按 `Ctrl+C`)，然后重新执行启动命令，如 `yarn start`。
