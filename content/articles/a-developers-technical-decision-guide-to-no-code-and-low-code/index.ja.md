...
Step 1: ビジネスの構造　業務ルールをテーブルやワークフロー図で無理なく表現できるか？　できない → 不向き

Step 2: 画面の複雑さ　画面に中程度以上の複雑な UI（高度なフォーム、特殊なビューなど）が必要か？　必要 → 不向き

Step 3: 性能要件　リアルタイム応答（100ms 未満）や高負荷処理など、細かい性能チューニングが求められるか？　求められる → 不向き

Step 4: 拡張の見通し　今後 6 ヵ月の拡張や追加要件をある程度予測でき、モジュールとして整理できるか？　できない → 慎重に利用

Step 5: チーム体制　プラットフォーム中心の開発フローを受け入れ、設定管理を運用する体制があるか？　ない → 不向き
