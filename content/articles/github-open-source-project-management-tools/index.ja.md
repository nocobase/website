...
[NocoBase](https://www.nocobase.com/) は、拡張性に優れたオープンソースの AI 駆動型ノーコードプラットフォームです。あらかじめ固定された機能を持たず、柔軟なデータモデルとプラグイン機構を提供し、AI 駆動の機能と組み合わせることで、チームが要件に応じてプロジェクト管理に必要なモジュールを自由に構築できます。
...
* **多様なタスクビュー**：NocoBase は、表、カンバン、ガントチャート、カレンダーなど、複数のタスクビューを同時にサポートしています。チームはプロジェクトのフェーズに応じて柔軟に切り替えることができ、計画段階ではガントチャートでマイルストーンや依存関係を整理し、実行段階ではカンバンでイテレーションのバックログを管理し、テスト段階では表形式で不具合や対応状況を一元的に把握できます。これらを基盤として、AI 駆動の「AI 従業員」が現在のビュー構造や選択中のタスクデータを理解し、実務を支援します
