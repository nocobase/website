...
你可以直接复制下方提示词，让 AI Agent 在 NocoBase 中生成考勤系统的数据模型和基础结构，再通过无代码方式调整数据模型、字段、页面、权限、考勤状态和统计规则。
...
NocoBase 可以将员工、部门和考勤记录关联成统一的考勤数据模型，并在同一个企业业务系统中配置移动端打卡页面、月度考勤矩阵、考勤明细、角色权限和统计看板。团队可以根据自己的上下班时间和考勤规则调整字段、状态和展示方式。
