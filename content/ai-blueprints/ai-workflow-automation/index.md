...
These records can also be managed as a NocoBase-based internal business system, with business triggers, condition checks, LLM nodes, AI employee nodes, record updates, and execution history organized through configurable data models, role permissions, views, workflows where needed, and operation history.

You can copy the prompt below and let your AI agent generate the basic structure of the AI workflow automation system in NocoBase, then visually adjust triggers, conditions, AI nodes, follow-up actions, permissions, and execution views.
...
For AI workflow automation, the AI Agent can first draft collections, relationships, prompts, and analysis logic around business triggers, condition checks, LLM nodes, AI employee nodes, record updates, and execution history. NocoBase then keeps the data model, permissions, workflows, pages, and operation history maintainable after launch.
