...
这些数据也可以作为基于 NocoBase 的企业内部业务系统来维护，将业务触发器、条件判断、LLM 节点、AI 员工节点、记录更新和执行历史放在可配置的数据模型、角色权限、页面、必要的工作流和操作历史中管理。

你可以直接复制下方提示词，让 AI Agent 在 NocoBase 中生成 AI 工作流自动化系统的基础结构，再通过可视化方式调整触发器、条件、AI 节点、后续操作、权限和执行视图。
...
在AI 工作流自动化系统场景中，AI Agent 可以先拆出围绕业务触发器、条件判断、LLM 节点、AI 员工节点、记录更新和执行历史的数据表、字段关系、流程节点和分析提示词；NocoBase 则负责承接数据模型、权限、工作流、页面和操作历史，便于系统上线后继续维护。
...
2. **AI 工作流和 Vibe Coding 生成的自动化脚本有什么不同？**
