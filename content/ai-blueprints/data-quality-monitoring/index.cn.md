...
这些数据也可以作为基于 NocoBase 的企业内部业务系统来维护，将数据质量规则、校验结果、异常记录、数据负责人、监控看板和问题趋势放在可配置的数据模型、角色权限、页面、必要的工作流和操作历史中管理。
...
在数据质量监控系统场景中，AI Agent 可以先拆出围绕数据质量规则、校验结果、异常记录、数据负责人、监控看板和问题趋势的数据表、字段关系、流程节点和分析提示词；NocoBase 则负责承接数据模型、权限、工作流、页面和操作历史，便于系统上线后继续维护。
