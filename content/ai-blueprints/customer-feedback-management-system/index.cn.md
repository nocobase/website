...
这些数据也可以作为基于 NocoBase 的企业内部业务系统来维护，将客户反馈、来源渠道、评分、产品建议、问题分类、响应状态和反馈分析放在可配置的数据模型、角色权限、页面、必要的工作流和操作历史中管理。
...
在客户反馈管理系统场景中，AI Agent 可以先拆出围绕客户反馈、来源渠道、评分、产品建议、问题分类、响应状态和反馈分析的数据表、字段关系、流程节点和分析提示词；NocoBase 则负责承接数据模型、权限、工作流、页面和操作历史，便于系统上线后继续维护。
